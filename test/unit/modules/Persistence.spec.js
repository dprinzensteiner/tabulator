...
describe("Persistence module", () => {
    ...
    it("should set up sessionStorage persistence mode when enabled", () => {
        // Set persistence option to true with sessionStorage available
        mockTable.options.persistence = true;
        persistence.localStorageTest.mockReturnValue(false);
        mockTable.options.persistenceMode = "session";

        // Initialize persistence
        persistence.initialize();

        // Verify that the mode is set to 'session'
        expect(persistence.mode).toBe("session");
        expect(persistence.readFunc).toBe(defaultReaders.session);
        expect(persistence.writeFunc).toBe(defaultWriters.session);
    });
